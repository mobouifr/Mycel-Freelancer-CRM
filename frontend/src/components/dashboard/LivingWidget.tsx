  ...
  return (
    <div style={{
      ...
      >
        ...
        {/* Trunk */}
        <line
          x1="50" y1="90" x2="50" y2={Math.max(30, 90 - growth * 0.6)}
          stroke="var(--text-dim)"
          strokeWidth="2.5"
          strokeLinecap="round"
          style={{
        ...
        {mushrooms.map((m, i) => (
          <g key={`m-${i}`} style={{ animation: `fadeIn .5s var(--ease) ${m.delay}s both` }}>
            {/* Stem */}
            <rect
              x={m.x - 1.5} y={90 - m.stemHeight}
              width={3} height={m.stemHeight}
              rx={1}
            ...
            {/* Cap */}
            <ellipse
              cx={m.x} cy={90 - m.stemHeight}
              rx={m.capRadius} ry={m.capRadius * 0.55}
        ...
        {/* Glow */}
        <circle
          cx="50" cy={Math.max(35, 85 - growth * 0.5)}
          r={8 + growth * 0.1}
          fill="var(--accent)"
          opacity="0.04"
      ...
      <div style={{ textAlign: 'center' }}>
        <p style={{
          ...
          {t('livingWidget.level', {
            level,
            growth: hours < 1 ? t('livingWidget.justPlanted') : t('livingWidget.hoursGrowth', { hours: Math.floor(hours) }),
          })}
        </p>
        <p style={{
